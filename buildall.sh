...
if ! test -d "$ANDROID_HOME/build-tools/$build_tools_version"; then
...
if ! test -d "$ANDROID_HOME/platforms/android-$min_api_level"; then
    add_package "platforms;android-$min_api_level"
fi

if ! test -d "$ANDROID_HOME/platforms/android-$max_api_level"; then
    add_package "platforms;android-$max_api_level"
fi

install_android_packages() {
    if test $# -ne 0; then
        if test -n "$JAVA8_HOME" && test -d "$JAVA8_HOME/bin"; then
            local PATH="$JAVA8_HOME/bin:$PATH"
        fi
        sdkmanager "$@"
    fi
}

install_android_packages "${packages_to_install[@]}"

which sdkmanager || { echo "'android' tool not found in your PATH" && exit 1 ; }
